...
export default function EntityDetailPage() {
  ...
  const [trackHeatmap, setTrackHeatmap] = useState<Record<string, number> | null>(null);
  ...
  const load = async () => {
    ...
    try {
      ...
      if (e.status === "fulfilled" && e.value.tracking?.enabled) {
        ...
        trackingService.heatmap(id!).then(setTrackHeatmap).catch(() => {
          setTrackHeatmap(null);
        });
      }
    } catch {
  ...
  return (
    <div className="space-y-4 max-w-2xl mx-auto">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="p-2 text-[#444] hover:text-[#3ecf8e] hover:bg-[#3ecf8e]/5 rounded-md transition-colors"
              title="Editar"
            >
              ✏️
            </button>
      ...
      {/* Heatmap (tracking) */}
      {isTrackable && trackHeatmap && Object.keys(trackHeatmap).length > 0 && (
        <div className="bg-[#111] border border-white/5 rounded-xl p-4">
          <p className="text-xs font-medium text-[#555] uppercase tracking-wider mb-3">
            Atividade (90 dias)
          </p>
          <HeatmapGrid heatmap={trackHeatmap} days={90} />
        </div>
      )}
      ...
      {timeline && (
        <div className="bg-[#111] border border-white/5 rounded-xl p-4">
          ...
          {/* Mention heatmap */}
          {Object.keys(timeline.heatmap).length > 0 && (
            <div className="mb-4">
      ...
      {editOpen && entity && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4">
          <div className="bg-[#111] border border-white/10 rounded-xl p-6 w-full max-w-md">
            <h3 className="font-semibold mb-3">Editar entidade</h3>
            <form onSubmit={async (e) => {
              ...
              try {
                ...
                if (form.color !== undefined) updateData.color = form.color;
                if (form.trackingEnabled !== undefined) {
                  updateData.tracking = {
              ...
              <div>
                <label className="text-xs text-[#666] mb-1 block">Nome</label>
                <input
                  value={form.name}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-[#666] mb-1 block">Ícone</label>
                  <input
                    value={form.icon || ""}
                ...
                <div>
                  <label className="text-xs text-[#666] mb-1 block">Cor</label>
                  <input
                    type="color"
                    value={form.color || "#3ecf8e"}
                    onChange={(e) => setForm((f) => ({ ...f, color: e.target.value }))}
                    className="w-full h-10 bg-[#1a1a1c] border border-white/10 rounded-md px-2 cursor-pointer"
                  />
                </div>
              </div>
              {/* tracking settings */}
              ...
              <div className="flex gap-2 justify-end">
                <button type="button" onClick={() => setEditOpen(false)} className="px-4 py-2 text-sm text-[#888] hover:text-white">Cancelar</button>
                <button type="submit" className="px-4 py-2 text-sm bg-[#3ecf8e]/20 text-[#3ecf8e] hover:bg-[#3ecf8e]/30 rounded-md">Salvar</button>
              </div>
            </form>
          </div>
        </div>
